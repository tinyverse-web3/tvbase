...
func Serve(node tvCommon.TvBaseService, lis net.Listener, options ...ServeOption) error {
	...
	serverProc := Process.Go(func(p goprocess.Process) {
	...
	case <-Process.Closing():
		Logger.Infof("server at %s terminating...", addr)

		warnProc := periodicproc.Tick(5*time.Second, func(_ goprocess.Process) {
			Logger.Infof("waiting for server at %s to terminate...", addr)
		})
	...
	Logger.Infof("server at %s terminated", addr)
	return serverError
}
