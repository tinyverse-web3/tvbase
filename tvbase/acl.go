...
func NewACL(h host.Host, cfg config.ACLConfig) (*ACLFilter, error) {
	acl := &ACLFilter{}
	...
	if len(cfg.AllowPeers) > 0 {
		...
		for _, s := range cfg.AllowPeers {
			p, err := peer.Decode(s)
			if err != nil {
				tvLog.Logger.Errorf("acl->NewACL: error decoding peer ID: %v", err)
				return nil, err
			}
	...
	if len(cfg.AllowSubnets) > 0 {
		...
		for _, s := range cfg.AllowSubnets {
			...
			if err != nil {
				tvLog.Logger.Errorf("acl->NewACL: error parsing subnet: %v", err)
				return nil, err
			}
